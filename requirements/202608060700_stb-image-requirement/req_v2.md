...
- **完整库版本会追加 vendoring `stb_image_write.h`**（同仓库姊妹头文件），脚本应预留扩展能力
...
- **仅支持 native 目标**：MVP 不暴露 wasm/js/wasm-gc 构建。`moon.mod` 设置 `preferred_target = "native"`（新格式，下划线；旧 `"preferred-target"` 已弃用），`ffi.mbt` 通过 `moon.pkg` 的 `targets` 门控到 native，可设 `supported_targets = "native"` 声明包级支持范围。解锁计划：v1.0 评估多目标支持（见第六节演进考量）
