...
- `options("native-stub": ["wrapper.c"])`：仅列 wrapper.c（stb_image.h 通过 #include 纳入）
- `options(targets: { "ffi.mbt": ["native"], "image_load_native.mbt": ["native"], "README.mbt.md": ["native"] })`：FFI 相关文件门控到 native
- `image_types.mbt`、`image_test.mbt` **不门控**：类型定义全后端可用，测试在所有后端运行（测试调用 load_from_* 会因 native 门控而在非 native 后端跳过或报错，需在测试侧处理，见 §7.2）

**门控清单**（已核实 make-moonbit-c-bindings skill 模板 + llvm.mbt 先例）：
| 文件 | 门控 | 理由 |
|------|------|------|
...
| `image_test.mbt` | 不门控 | 测试应在所有后端运行（但 load 测试需 native，见 §7.2） |
...
**测试文件门控**：
- `image_test.mbt`：**不门控**（测试在所有后端运行）
- 但 `load_from_*` 在非 native 后端不可用，测试调用会编译失败
- **解决方案**：测试文件门控到 `["native"]`（`targets: { "image_test.mbt": ["native"] }`），因为测试核心是验证 FFI 行为，仅 native 有意义
- 或在测试中用条件编译跳过非 native 后端（但 MoonBit 条件编译以文件为单位，故采用文件门控）

**修正 §3.3 门控清单**：`image_test.mbt` 应门控到 `["native"]`（测试调用 `load_from_*`，仅 native 可用）。
