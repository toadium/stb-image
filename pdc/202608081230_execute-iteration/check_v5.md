...
| 检查项 | 方法 | 结果 |
|--------|------|------|
| 产出文件存在性 | 读取 `src/pure/{codec,pixel,color,process,util}/qoi_decode.mbt`（119 行）、`src/pure/{codec,pixel,color,process,util}/qoi_decode_test.mbt`（267 行）、`src/roundtrip_test.mbt:116` 新增测试 | 通过：三处产出均存在且非空 |
...
| 移植正确性（@core→@types） | 对比 `src/pure/{codec,pixel,color,process,util}/qoi_decode.mbt` 与 `src/format/qoi.mbt:13-116` 逻辑 | 通过：仅类型引用 `@core.Image`→`@types.Image`、`@core.LoadError`→`@types.LoadError` 替换，解码逻辑完全保留 |
...
| 对比测试 API 引用正确性 | 审查 `roundtrip_test.mbt:121-123` | 通过：使用 `@format.encode_qoi`/`@format.decode_qoi`/`@codec.decode_qoi_pure`（非 `@core.encode_qoi`），符合 task_v5.md r1 修正要求 |
| 对比测试交叉验证逻辑 | 审查 `roundtrip_test.mbt:125-128` | 通过：断言 width/height/channels/data 完全一致，覆盖 RGB 路径（req_channels=Some(3)） |
| pure 包全目标化 | 读取 `src/pure/{codec,pixel,color,process,util}/moon.pkg` | 通过：仅 `import types`，无 `supported_targets` 限制，全目标可用 |
