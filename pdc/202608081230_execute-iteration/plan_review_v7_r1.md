...
- pure 包 `src/pure/moon.pkg` 仅 `import types`，无 `supported_targets`，全目标可用
...
- 对比测试类型匹配：`@format.encode_ppm`/`encode_pgm` 接受 `@core.Image`，`@core.load_from_path` 返回 `@core.Image`；`@pure.decode_pnm_pure` 返回 `@types.Image`，`@core.Image` 即 `@types.Image`（re-export），字段级比较可行
