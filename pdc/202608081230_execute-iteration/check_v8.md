...
| 检查项 | 方法 | 结果 |
|--------|------|------|
| 解码器签名合规 | 读取 `src/pure/psd_decode.mbt:30` 验证 `pub fn decode_psd_pure(data : Bytes) -> @types.Image raise @types.LoadError` | 通过 — 与 BMP/QOI/TGA/PNM 惯例一致 |
...
| 纯逻辑测试数量与命名 | 读取 `src/pure/psd_decode_test.mbt`，统计 `test` 块 | 通过 — 13 个（4 正例 + 9 错误路径），命名与任务要求一致 |
...
| pure 包全目标可用 | 检查 `src/pure/moon.pkg` 仅 `import types`，无 `supported_targets` 限制 | 通过 — pure 包全目标，PSD 解码器仅依赖 @types |
