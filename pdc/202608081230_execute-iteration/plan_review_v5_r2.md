...
4. **预期测试数 562**：当前 native 553（check_v4.md 确认）。pure 包全目标化（`src/pure/{codec,pixel,color,process,util}/moon.pkg` 无 supported_targets），native 为其目标之一，moon test --target native 必运行 pure 包测试（T3 先例：T3 后 native 552 已含 pure 6 测试）。新增 pure 8 测试 + 根包 1 测试 = 553+8+1=562，计算正确，不再有"另计"模糊表述。
...
6. **类型兼容**：`@codec.decode_qoi_pure` 返回 `@types.Image`，`@format.decode_qoi` 返回 `@core.Image`（即 `@types.Image` 别名，`src/core/image_types_reexport.mbt`），字段 width/height/channels/data 均 derive(Eq)，字段级比较可行。
7. **依赖可用性**：根包 `src/moon.pkg` 第 11 行 import format（主列表）、第 16-18 行 `for "test"` 声明 @pure。roundtrip_test.mbt 为 native-only（`options(targets: {"roundtrip_test.mbt": ["native"]})`），可自由依赖 @core+@format+@pure。现有 `roundtrip: BMP RGB pure vs FFI`（第 45-60 行）用 `@codec.decode_bmp_pure`、`roundtrip: QOI RGB`（第 90-100 行）用 `@format.encode_qoi`/`@format.decode_qoi`，均印证可用。
8. **pure 包现状**：`src/pure/{codec,pixel,color,process,util}/` 当前仅 bmp_decode.mbt + bmp_decode_test.mbt（glob 确认），moon.pkg 仅 import types 全目标。新增 qoi_decode.mbt + qoi_decode_test.mbt 仅依赖 @types，全目标可用，不破坏现有结构。
