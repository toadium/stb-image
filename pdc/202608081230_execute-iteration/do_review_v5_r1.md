...
### 任务覆盖度
- **[通过]** 4 项预期产出全部交付：
  1. `src/pure/{codec,pixel,color,process,util}/qoi_decode.mbt` 已创建，实现 `pub fn decode_qoi_pure(data : Bytes) -> @types.Image raise @types.LoadError`，支持 RGB（channels=3）和 RGBA（channels=4）解码
  2. `src/pure/{codec,pixel,color,process,util}/qoi_decode_test.mbt` 已创建，8 个纯逻辑测试，覆盖全部 6 种 QOI 标签 + 2 错误路径
...
### 正确性
- **[通过]** 解码器移植忠实：`src/pure/{codec,pixel,color,process,util}/qoi_decode.mbt` 与参考实现 `src/format/qoi.mbt:13-116` 逐行对比完全一致，仅替换 `@core.Image`→`@types.Image`、`@core.LoadError`→`@types.LoadError`，逻辑无改动
...
- **[通过]** 交叉验证测试逻辑正确：`@format.encode_qoi` → `@codec.decode_qoi_pure` vs `@format.decode_qoi`，断言 width/height/channels/data 完全一致
...
### 一致性
- **[通过]** pure 包 `src/pure/{codec,pixel,color,process,util}/moon.pkg` 仅 `import types`，无 `supported_targets`，全目标化，QOI 解码器仅依赖 @types，与 T3 架构一致
