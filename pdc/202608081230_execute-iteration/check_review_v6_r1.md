...
- **[轻微]** 检查报告未显式说明"pure 包不涉及 FFI 因此无需 ASan 验证"。任务约束第 5 条要求"FFI 部分 ASan 验证"，本任务新增的是纯 MoonBit 解码器（`src/pure/{codec,pixel,color,process,util}/` 仅 `import types`，无 FFI 依赖），从上下文显然不适用，未构成遗漏。
...
7. pure 包全目标：`src/pure/{codec,pixel,color,process,util}/moon.pkg` 仅 `import types`，无 `supported_targets` 限制。
