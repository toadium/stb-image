...
## 任务描述
在 `src/pure/{codec,pixel,color,process,util}/` 新增纯 MoonBit PNM 解码器，扩展 pure 包格式覆盖（BMP+QOI+TGA → BMP+QOI+TGA+PNM），推进 v2.0 多目标支持。

### 实现要求
1. **解码器文件** `src/pure/{codec,pixel,color,process,util}/pnm_decode.mbt`：
...
2. **纯逻辑测试文件** `src/pure/{codec,pixel,color,process,util}/pnm_decode_test.mbt`（全目标，不依赖 @core）：
   - 手构造 PNM 字节流验证解码正确性
   - 测试覆盖建议 7-9 个，含：
...
3. **FFI 基准对比测试**（根包 `src/roundtrip_test.mbt`，native-only）：
   - 新增 2 个测试：
     - `roundtrip: PPM RGB pure vs FFI`：`@core.load_from_path("testdata/test_4x4_red.png", req_channels=Some(3))` → `@format.encode_ppm(img)` → `@codec.decode_pnm_pure(encoded)` vs `@core.load_from_bytes(encoded, req_channels=Some(3))` → 断言 width/height/channels/data 完全一致
     - `roundtrip: PGM grayscale pure vs FFI`：构造 2x2 灰度 Image → `@format.encode_pgm(img)` → `@codec.decode_pnm_pure(encoded)` vs `@core.load_from_bytes(encoded, req_channels=Some(1))` → 断言 width/height/channels/data 完全一致
   - 使用 `@format.encode_ppm`/`encode_pgm`（非 `@core.`，core 包无此二函数，见 `src/format/pnm_encode.mbt:6,37`，根包已 import format）

### 预期产出
- `src/pure/{codec,pixel,color,process,util}/pnm_decode.mbt` 新建
- `src/pure/{codec,pixel,color,process,util}/pnm_decode_test.mbt` 新建
