...
[![Tests](https://img.shields.io/badge/tests-987%20%C3%97%204%20targets-brightgreen)]()
[![Coverage](https://img.shields.io/badge/coverage-89.8%25-brightgreen)]()
[![Functions](https://img.shields.io/badge/API-276%20functions%20%2B%2045%20types-blueviolet)]()
...
> [!NOTE]
> 四目标（native / wasm-gc / js / wasm）均使用同一套纯 MoonBit 代码，各 987 测试全部通过，覆盖率 89.8%。
...
| 🟢 | **276 个 API** | 从基础 I/O 到 FFT、Canny、分水岭、SLIC、ORB、SIFT、光流、模板匹配等高级算法 |
...
| 目标 | 后端 | 测试 | 状态 |
|:----:|:----:|:----:|:----:|
| **native** | 纯 MoonBit | 987 | ✅ |
| **wasm-gc** | 纯 MoonBit | 987 | ✅ |
| **js** | 纯 MoonBit | 987 | ✅ |
| **wasm** | 纯 MoonBit | 987 | ✅ |
...
├── bench.mbt           # 性能基准测试
└── reexport.mbt        # 顶层 API re-export (276 pub fn + 45 pub type)
```
...
| [docs/api_reference.md](docs/api_reference.md) | 完整 API 参考（276 函数 + 45 类型） |
...
cd stb-image
moon check                          # 编译检查
moon test --target native           # 运行测试（应 987 通过）
```
